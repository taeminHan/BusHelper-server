...
class Bus:
    ...
    def FindRoute(self):
        ...
        # 출력확인용
        print('버스번호', routeBusNumber)

        # 노선번호 (환승필요시 2개 이상의 버스 리스트 출력) 데이터타입 = list
        routeRouteId = re.compile(r"\d+\w").findall(str(soup.select_one('itemlist').select('pathList>routeId')))
        # 출력확인용
        print('노선번호', routeRouteId)
    ...
    def FindStation(self):
        ...
        print("도착 정류장번호", re.findall(r"\d+.\d+", str(end_soup.select_one('poiId')))[0])
